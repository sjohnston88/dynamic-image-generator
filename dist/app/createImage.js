...
const createImage = async (req, res) => {
  ...
  _canvas.default.registerFont(fontPath, {
    family: 'Indie Flower'
  });
  ...
  const ctx = canvas.getContext('2d'); // Configure background

  const background = await _canvas.default.loadImage(backgroundPath);
  ctx.drawImage(background, 0, 0, canvas.width, canvas.height); // Configure text
